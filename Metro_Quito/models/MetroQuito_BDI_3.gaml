...
species tren skills: [moving] {
	...
	estacion origen_base; // Su "casa"
	estacion destino_base; // Su destino opuesto
	
	estacion target_station; // Hacia donde va ahora mismo
	path camino_actual;
	...
	action iniciar_viaje (estacion destino_objetivo) {
		...
		// Calculamos el camino usando el grafo del metro
		camino_actual <- path_between(red_metro, location, target_station.location);
		
		// Debug para ver si encuentra camino
		if (camino_actual = nil) {
			write "ERROR: No hay camino entre " + location + " y " + target_station.location;
			estado <- "en_cochera"; // Abortar
		}
	}

	reflex movimiento when: estado = "movimiento" {
		// /// CORRECCIÓN 5: MOVIMIENTO ROBUSTO ///
		if (camino_actual != nil) {
		...
		if (self distance_to est < 60) { // Aumenté un poco el radio de detección
			do gestionar_pasajeros(est);
		}

		// Llegada al destino final de la ruta actual
		if (location distance_to target_station < 60) {
			// Si llegó al destino (ej. Labrador), debe regresar a su origen (ej. Quitumbe)
			if (target_station = destino_base) {
				// Llegó al final, ahora regresa
				do iniciar_viaje(origen_base);
			} else {
				// Llegó de regreso a su base (Ciclo completo)
				// Descarga a todos
				ask passengers_onboard { do die; }
				passengers_onboard <- [];
				
				// Se pone en cola para salir de nuevo cuando el despachador diga
				estado <- "en_cochera";
				camino_actual <- nil;
			}
		}
	...
	action gestionar_pasajeros (estacion est) {
		...
		// Solo suben pasajeros que van en la dirección correcta
		// (Simplificación: suben todos los que pueden, en una simulación avanzada filtraríamos dirección)
	...
	aspect base {
		if (estado != "en_cochera") {
			...
			// Dibujar los trenes en cochera más pequeños
			draw circle(20) color: #gray at: location;
		}
	}
}
